...
minimize ``rmse_validation`` from :func:`Mains.multislice`. Runs until a
wall-clock time budget (default 1 hour).

Differences from ``evolve_hyperparams.py`` (v1):

- Drives ``Mains.multislice`` instead of ``Mains.general``.
- Search space restricted to K, K_mode, mean_nbin, deriv1st_nbin, taumax.
  outagain_gen is dropped (free generation disabled in multislice-mode) and
  trnpat is dropped from the search (it is read from the base .par and held
  fixed as the per-ticker per-block training length).
- Objective is fixed to rmse_validation (no --objective flag). Because
  ``Mains.multislice`` returns a per-(ticker, phase, block) RMSE cube rather
  than scalar RMSEs, the scalar objective reported here is the NaN-mean over
  all valid validation entries (``np.nanmean(result["rmse"][:, 1, :])``); the
  train / test / persistence figures are aggregated the same way.
- ``timeseries_file`` (and optional ``tickers``) from the base .par file define
  the multi-ticker walk-forward setup and are preserved on every evaluation; the
  EA does not mutate them. Empty or missing ``tickers`` uses all CSV columns.

Run from the ``general`` directory (or pass --chdir). Default --base is
``Parameters_multislice.par`` (wide-CSV ``timeseries_file`` + ``tickers``
list; per-ticker per-block ``trnpat``/``vanpat``/``tenpat``).
Example:

    python evolve_hyperparams_v2.py --time 3600 \\
        --base Parameters_multislice.par \\
...
def apply_genes(base_params: Dict[str, Any], g: Genes) -> Dict[str, Any]:
    ...
    timeseries_file / tickers / trnpat / vanpat / tenpat and all other base
    keys are preserved verbatim so Mains.multislice sees the same multi-ticker
    walk-forward setup on every evaluation."""
    p = dict(base_params)
    p["K"] = g.K
    p["K_mode"] = g.K_mode
...
def _nanmean_cube(arr: np.ndarray) -> float:
    ...
    Returns NaN (rather than warning) when no finite entries are present."""
...
def evaluate(
    ...
    # Mains.multislice has its own internal stochasticity; repeat n_repeats
    # times and average per-metric RMSEs to smooth noise (matches v1 convention).
    ...
    for _ in range(max(1, int(n_repeats))):
        try:
            ...
            continue
        # Mains.multislice returns a dict with a [n_tickers, 3, n_blocks] RMSE
        # cube (phase axis = train, val, test) plus a matching persistence cube.
        # Collapse each to a scalar via NaN-mean over all valid entries.
...
def run_ea(
    ...
    if "timeseries_file" not in base_params:
        raise KeyError(
            ...
            f"'timeseries_file'. evolve_hyperparams_v2.py drives "
            f"Mains.multislice(), which requires timeseries_file (wide CSV); "
            f"tickers is optional (comma-separated column list, or omit for "
            f"all columns). Use a Parameters_multislice-style file (or fall "
            f"back to v1 / Mains.general).")
    cache: Dict[Tuple[Any, ...], Dict[str, Any]] = {}
...
def main() -> None:
    ...
    parser.add_argument("--base", default="Parameters_multislice.par",
                        help="Base .par template (must contain timeseries_file; "
                             "tickers optional); "
                             "default Parameters_multislice.par")
    parser.add_argument("--work-par", default="Parameters_multislice_ea_work.par",
